...
public class InterfazRetirada extends JFrame {
	DatosUsuario DU = new DatosUsuario();
	private static final long serialVersionUID = 1L;
	JTextField tf1, tf2, tf3; // Se generan los campos de texto
	JLabel label1, label2, label3;// Se generan los textos
	JButton Retirar, Cancelar, Salir;// se generan los botones

	public InterfazRetirada() {
		...
		pantalla.add(label3);
		pantalla.add(tf3);
		pantalla.add(Retirar);
		pantalla.add(Cancelar);
		pantalla.add(Salir);

		// Acciones para los botones
		Retirar.addActionListener(new event());
	...
	private class event implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().equals("Retirar")) {
				...
				// cant = Double.parseDouble(tf3.getText());
